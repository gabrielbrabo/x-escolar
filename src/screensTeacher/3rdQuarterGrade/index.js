...
import { clssInfo, RegisterNumericalGrade, getIIIrdQuarter, GetNumericalGradeIIIRdQuarter, updateNumericalGrade } from '../../Api'
...
const IndexAttendance = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (id_matter && year && id_iiiRdQuarter && id_class) {
                //setLoading(true);
                const resGrade = await GetNumericalGradeIIIRdQuarter(year, id_matter, id_iiiRdQuarter, id_class)
                const resClass = await clssInfo(id_class)
    ...
    }, [year, id_iiiRdQuarter, id_matter, averageGrade, totalGrade])
