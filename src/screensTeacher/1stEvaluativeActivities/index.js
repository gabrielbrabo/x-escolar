...
const IndexAttendance = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (id_matter && year && id_bimonthly && id_class) {
                ...
                if (resActivity.data.data) {
                    ...
                    const contStudent =  await resClass.data.data.find(res => {
                        return res
                    }).id_student
                    ...
                    const totalNotas =  await resActivity.data.data.reduce((sum, activity) => {
    ...
    }, [year, id_iStQuarter, id_matter, averageGrade, totalGrade, bimonthly,])
