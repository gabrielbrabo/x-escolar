...
import { clssInfo, RegisterGradeIIndQuarter, getIIndQuarter, GetGradeIINdQuarter, updateGrade, GetMatter, GetMatterDetails } from '../../Api'
...
const IndexAttendance = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            const regime = IIndQuarter.data.data?.[0]?.assessmentRegime;
            ...
            if (id_mttr && currentYear && id_iiNdQuarter && id_class) {
                setLoading(true);
                const resGrade = await GetGradeIINdQuarter(currentYear, id_mttr, id_iiNdQuarter, id_class)
                const resClass = await clssInfo(id_class)
    ...
    }, [year, id_iiNdQuarter, id_matter, employee])
    ...
    /*const handleGrade = async (stdt) => {
        ...
        console.log("year", year, "bimonthly", bimonthly, "totalGrade", totalGrade, "averageGrade", averageGrade, "studentGrade", studentGrade, "id_iiNdQuarte", id_iiNdQuarter, "id_student", id_student, "id_teacher", id_teacher, "id_matter", id_matter)
        if (RegentTeacher === id_teacher) {
            const id_teacher02 = RegentTeacher02
            const res = await RegisterGradeIIndQuarter(year, bimonthly, studentGrade, id_iiNdQuarter, id_student, RegentTeacher, id_teacher02, id_matter, id_class)
            if (res) {
                const resGrade = await GetGradeIINdQuarter(year, id_matter, id_iiNdQuarter, id_class)
                const resClass = await clssInfo(id_class)
                const GradeRealized = await resGrade.data.data.map(res => {
            ...
            const res = await RegisterGradeIIndQuarter(
                year,
                bimonthly,
                studentGrade,
                id_iiNdQuarter,
    ...
    const handleFinalize = async () => {
        ...
        try {
            const promises = toSend.map(s => {
                console.log("year", year, "bimonthly", bimonthly, "totalGrade", totalGrade, "averageGrade", averageGrade, "id_iiNdQuarter", id_iiNdQuarter, "id_student", s._id, "id_teacher", id_teacher, "id_matter", id_matter)
                ...
                return RegisterGradeIIndQuarter(
                    year,
                    bimonthly,
                    s.concept,
                    id_iiNdQuarter,
