...
import { clssInfo, RegisterNumericalGrade, getIIndQuarter, GetNumericalGradeIINdQuarter, updateNumericalGrade } from '../../Api'
...
const IndexAttendance = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (id_matter && year && id_iiNdQuarter && id_class) {
                //setLoading(true);
                const resGrade = await GetNumericalGradeIINdQuarter(year, id_matter, id_iiNdQuarter, id_class)
                const resClass = await clssInfo(id_class)
    ...
    }, [year, id_iiNdQuarter, id_matter, averageGrade, totalGrade])
