...
import { clssInfo, RegisterGradeVIthQuarter, getVIthQuarter,  GetGradeVIthQuarter } from '../../Api'
...
const IndexAttendance = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (id_matter && year && id_viThQuarter) {
                setLoading(true);
                const resGrade = await GetGradeVIthQuarter(year, id_matter, id_viThQuarter)
                const resClass = await clssInfo(id_class)
    ...
    const handleGrade = async (stdt) => {
        ...
        const res = await RegisterGradeVIthQuarter(year, bimonthly, totalGrade, averageGrade, studentGrade, id_viThQuarter, id_student, id_teacher, id_matter)
        if (res) {
            const resGrade = await  GetGradeVIthQuarter(year, id_matter, id_viThQuarter)
            const resClass = await clssInfo(id_class)
            const GradeRealized = await resGrade.data.data.map(res => {
