...
import { GetNumGrade, AttendanceBimonthly, indexNumericalGradesCard, GetLogo, getIstQuarter, getIIndQuarter, getIIIrdQuarter, fetchLogo } from '../../Api';
...
const GradeIstquarter = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const tgIIIrd = await IIIrdQuarter.data.data.map(res => {
        return res.totalGrade
      })
      const agIIIrd = await IIIrdQuarter.data.data.map(res => {
        return res.averageGrade
      })
      ...
      if (resGrade && resGrade.data && Array.isArray(resGrade.data.data) && resGrade.data.data.length > 0) {
        ...
        if (res && res.id_ivThQuarter) {
          ...
          console.log('totalGrade', res.id_ivThQuarter.totalGrade)
          console.log('averageGrade', res.id_ivThQuarter.averageGrade)

          const tg = res.id_ivThQuarter.totalGrade
          const ag = res.id_ivThQuarter.averageGrade
