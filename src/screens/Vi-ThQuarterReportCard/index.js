  ...
  useEffect(() => {
    (async () => {
      ...
      if (resGrade && resGrade.data && Array.isArray(resGrade.data.data) && resGrade.data.data.length > 0) {
        ...
        if (res && res.id_viThQuarter) {
          let startd = res.id_viThQuarter.startday || null;
          let startm = res.id_viThQuarter.startmonth || null;
          let starty = res.id_viThQuarter.startyear || null;

          let endd = res.id_viThQuarter.endday || null;
          let endm = res.id_viThQuarter.endmonth || null;
          let endy = res.id_viThQuarter.endyear || null;
